const ERROR_MESSAGES = {
  EXTENSION_COMMUNICATION: "拡張機能との通信でエラーが発生しました。ページを再読み込みしてください。",
...
const SUCCESS_MESSAGES = {
  ...
  CSV_IMPORTED_WITH_SKIP: "CSVをインポートしました（追加: {mergeResult.added}件, スキップ: {mergeResult.skipped}件）",
...
const CONFIRM_MESSAGES = {
  RESET_ALL_DATA: "すべてのデータをリセットしますか？この操作は取り消せません。",
...
const VALIDATION_MESSAGES = {
  ...
  MIN_LENGTH: "{fieldName}は{minLength}文字以上で入力してください（現在{length}文字）",
  MAX_LENGTH: "{fieldName}は{maxLength}文字以下で入力してください（現在{length}文字）",
  FILE_SIZE_TOO_LARGE: "ファイルサイズが大きすぎます。{maxSize}MB以下のファイルを選択してください。",
